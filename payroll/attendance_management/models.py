    ...
    def update_checkin_toggle(self):
        if self.check_in_toggle:
            ...
            if self.check_in_time==None:
                ...
                if timezone.now().time().hour > 9:self.is_late = True
...
class DailyAttendance(models.Model):
    ...
    def update_checkout(self):
        ...
        all_out = {obj:(True if value.check_in_toggle==False else False) for obj,value in self.employee_attendance_objects.items() }

        if reduce(lambda a,b:b==True,all_out.values()):
            for obj,value in self.employee_attendance_objects.items():value.update_checkout()
            self.save()
        else:
            raise ObjectsNotValidatedError(not_validated=dict(filter(lambda elem: elem[1] != True,all_out.items())).keys())

    def update_last_checkout(self):
        """
           Updates employees not checked out checkout time with last employee checkout.
        """

        all_out = {obj:(True if value.check_in_toggle==False else False) for obj,value in self.employee_attendance_objects.items() }

        not_checked_out = dict(filter(lambda elem: elem[1] != True,all_out.items()))
    ...
    def save(self,*args,**kwargs):
        if self.status==None:
            self.employee_attendance_objects = {f"{employee.full_name}":Attendance.objects.create(employee=employee) for employee in Employee.objects.all()}
