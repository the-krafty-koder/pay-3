        ...
        if form.is_valid():
            ...
            if user_auth is not None:
                if user_auth.is_active:
                    ...
                    return redirect('dashboard_payroll')  if isinstance(user_auth, (Users,)) else redirect('check_in_employee',
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user_auth is not None:
                if user_auth.is_active:
                    login(request, user_auth)
                    return redirect('employee_checkin',employee_name=user_auth.full_name) 
                return redirect('login',invalid="True")
    else:
        if invalid=="True":
            is_valid = "True"
