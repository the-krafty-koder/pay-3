...
def add_employee_documents(request, employee_name):
    ...
    if request.method == "POST":
        ...
        education_form, document_form = EducationForm(initial={"employee": employee}),
        DocumentForm(initial={"employee": employee})

        education, documents = Education.objects.filter(employee=employee.full_name),
        Document.objects.filter(employee=employee.full_name)
...
def delete_employee(request, employee_firstname, employee_lastname):
    ...
    deleted_employee = EmployeeBuilder("{} {}".format(employee_firstname,
                                                      employee_lastname)).delete_employee()
